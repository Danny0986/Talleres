...
Python tiene una sintaxis mas facil de comprender, ya que es lo mas cercano a un lenguaje natural, ademas de cer corto y claro.
...
getAverage = \nums -> fromIntegral (sum nums) / fromIntegral (length nums)
```
...
get_average = lambda nums: reduce(lambda x, y: x + y, nums) / len(nums)
```

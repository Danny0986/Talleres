...
1. Desarrolle una función que permita convertir un valor de temperatura de una escala a otra. La función principal deberá ser capaz de recibir la función de conversión deseada y el valor de temperatura a convertir.
  ...
  **Funciones de cálculo de referencia**
   ...
   **Solucion**
   ```scala
   // Ejercicio 4 Farenheit

   def convertirTemp(conversion: Double => Double, valor: Double): Double = {
     conversion(valor)
   }

   def convertion(option: Int): Double => Double = {
     option match {
       case 1 => (c: Double) => c * 9/5.0 + 32
       case 2 => (f: Double) => (f - 32) * 5/9.0
       case 3 => (c: Double) => c + 273.15
     }
   }

   convertirTemp(convertion(1), 25)

...
Solución:
```scala
def f3(n: Int) = (1 to n).map(i => 1/(2*i-1).toDouble).sum
...
Solución:
```scala
def f4(n: Int) = 0.5 * (1 to n).sum * (1 to n).product
...
Solución:
```scala
def f5(n: Int, k: Int) = k * (1 to n).sum
